...
class AuthenticationHelper:
    scope: str = "https://graph.microsoft.com/user.read"
    ...
    async def get_auth_claims_if_enabled(self, headers: dict) -> dict[str, Any]:
        ...
        try:
            ...
            roles = await self.validate_access_token(auth_token)
            ...
            graph_resource_access_token = self.confidential_client.acquire_token_on_behalf_of(
                user_assertion=auth_token, scopes=["https://graph.microsoft.com/user.read"]
            )
    ...
    async def validate_access_token(self, token: str):
        ...
        try:
            ...
            decoded_token = jwt.decode(token, rsa_key, algorithms=["RS256"], options={"verify_aud": False})
